...
export function watchToInsert(watch: Watch, userId: string): WatchInsert {
  return {
    ...
    market_value: watch.currentValue ?? null,
    market_source: watch.marketSource ?? null,
    market_confidence: watch.marketConfidence ?? null,
    market_updated_at: watch.marketUpdatedAt ?? null,
...
export function rowToWatch(row: WatchRow): Watch {
  return {
    ...
    currentValue: row.market_value ?? undefined,
    marketSource: (row.market_source as Watch['marketSource']) ?? undefined,
    marketConfidence: row.market_confidence ?? undefined,
    marketUpdatedAt: row.market_updated_at ?? undefined,
    condition:
